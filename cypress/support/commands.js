...
Cypress.Commands.add("alertaSuscr", () => {
  ...
  cy.get("body").then(($body) => {
    if (
      $body.find("button.swal2-confirm.swal2-styled.swal2-default-outline")
        .length > 0
    ) {
      ...
      cy.get("button.swal2-confirm.swal2-styled.swal2-default-outline")
        .contains("Si")
        .click(); // Reemplaza "#miOtroBoton" con tu selector real
    } else {
...
Cypress.Commands.add("alertaNotif", () => {
  cy.wait(420);
  cy.get("body").then(($body) => {
    const botonCerrar = $body.find(
      'button.swal2-confirm.swal2-styled.swal2-default-outline:contains("Cerrar")'
    );
...
Cypress.Commands.add("busquedaCliente", (tipoDocumento, InfoTipoDocumento) => {
  ...
  cy.get("body").then(() => {
    cy.wait(1500);
    cy.xpath(
      "/html/body/app-root/app-container/bac-app-container/div/mat-drawer-container/mat-drawer[1]/div/div[2]/mat-nav-list/div[1]/a/span/span/mat-icon[2]"
    ).then(($el) => {
      if ($el.length > 0) {
        cy.wrap($el).click();
      } else {
  ...
  cy.get("body").then(() => {
    cy.xpath(
      "/html/body/app-root/app-container/bac-app-container/div/mat-drawer-container/mat-drawer[1]/div/div[2]/mat-nav-list/div[1]/mat-nav-list/a[1]/span"
    ).then(($el) => {
      if ($el.length > 0) {
        cy.wrap($el).click();
      } else {
  ...
  cy.get("body").then(() => {
    cy.contains("mat-label", "Tipo de documento")
      .click({ force: true })
      ...
      .then(($el) => {
        if ($el.length > 0) {
          ...
          cy.get(".mat-mdc-input-element")
            .eq(1)
            .click()
            .type(InfoTipoDocumento); // Luego escribe el valor
        } else {
